...
!**********************************************************************
    ...
    IF(METCOLS(MHET).NE.MISSING) THEN
        print *, " Reading in crown evapotranspiration..."
        DO IHR = 1,KHRS
    ...
    !print *, solarRad
    DO IHR = 1,KHRS
            write (umetout,*) int(iDay), int(IHR), WINDAH(IHR), TAIR(IHR), TSOIL(IHR), RH(IHR), VPD(IHR), VMFD(IHR), CA(IHR), RADABV(IHR,1), SOLARRAD(IHR,1), FBEAM(IHR,1), PRESS(IHR), TDEW(IHR), SOILMOIST(ihr), PPT(IHR), tMin, tMax
    END DO
    RETURN
